...
def _best_side(landmarks, side_ids, required=None):
    """Pick the body side the camera sees best. Returns (side_index, points)
    for the side with the highest visibility, or None if neither is visible
    enough. `required` is how many of the triple's points must clear
    MIN_VISIBILITY (default: all of them); the side is ranked by the
    visibility of its `required`-th best point, so a partially-occluded side
    (e.g. ankle off-frame on a laptop webcam) still qualifies."""
    best = None
